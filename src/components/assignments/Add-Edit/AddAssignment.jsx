...
const AddAsignment = () => {
    ...
    console.log("dataRolesaaaaaaaaa0", dataRoles);
    ...
    const projectName = useSelector((state) => state.selectedRowProjectName.selectedRowProjectName);
    const projectNameEdit = useSelector((state) => state.selectedRowProjectNameEdit.selectedRowProjectNameEdit);
    ...
    useEffect(() => {
        if (isAddClicked) {
            ...
            setDataAssignment(prevFormData => ({
                ...
                nProjectName: dispatch(assignmentSlice.actions.setSelectedRowProjectNameEdit(null))
            }));
            //console.log("set", dataAssignment.Name)
        } else if (selectedRow) {
            dispatch(mainSlice.actions.setIsEditMode(true));

            const foundProject = dataAssignments.find((item) => {
            ...
            const foundProjectName = dataAssignments.find((item) => {
                return item?.ID === selectedRow?.ID;
            });

            console.log("foundProjectName",foundProjectName);

            const dataProjectNameSelected = foundProjectName ? foundProjectName : null;
            console.log("dataProjectSelected", dataProjectNameSelected);
            dispatch(assignmentSlice.actions.setSelectedRowProjectNameEdit(dataProjectNameSelected));
        ...
        <div className="grid" style={{ marginLeft: "20px", marginTop: "20px" }}>
            ...
            <div className="col-5">
                <DropdownComponentwithButton
                    ...
                    options={dataAssignments}
                    optionLabel="nProjectName"
                    label="Role"
                    onChange={(e) => {
                        console.log("aaaaaaaaaaaaaaa", e.value);
                        const selectedValue = e.value ? e.value.ID : null;
                        handleChange("nProjectName", selectedValue);
                        dispatch(assignmentSlice.actions.setSelectedRowProjectName(e.value))
                    }}
                  onButtonClick={DialoRole}
                //   showDialog={}
                //   hideDialog={}
